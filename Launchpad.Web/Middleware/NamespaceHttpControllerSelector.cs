...
namespace Launchpad.Web.Middleware
{
    public class NamespaceHttpControllerSelector : IHttpControllerSelector
    {
        ...
        public HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
            ...
            // Fall back to previous version if one exists.
            const string versionPrefix = "v";
            int currentVersion = int.Parse(namespaceName.ToLowerInvariant().Replace(versionPrefix, string.Empty));
            while (currentVersion > 0)
            {
                var decrementedVersion = currentVersion - 1;
                key = key.ToLowerInvariant().Replace($"v{currentVersion}", $"v{decrementedVersion}");
                if (_controllers.Value.TryGetValue(key, out controllerDescriptor))
                {
                    return controllerDescriptor;
                }
